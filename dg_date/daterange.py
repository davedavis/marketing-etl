...
def get_adobe_date_range(quarter):
    ...
    # adobe_single_date_range = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3]
    #
    # console.print("Current Date:   ", datetime.datetime.now(), style="Bold Purple")
    # console.print("Passed Date:    ", date_range, style="bold red")
    # console.print("Required:        2020-04-01T00:00:00.000/2020-04-02T00:00:00.000", style="Bold Green")
    # console.print("Received:        " + adobe_single_date_range, style="Bold Red")




    # if quarter == 1:
    #     adobe_start_date = company_fiscal_year.q1.start.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]
    #     adobe_end_date = company_fiscal_year.q1.end.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]
    # elif quarter == 2:
    #     adobe_start_date = company_fiscal_year.q2.start.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]
    #     adobe_end_date = company_fiscal_year.q2.end.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]
    # elif quarter == 3:
    #     adobe_start_date = company_fiscal_year.q3.start.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]
    #     adobe_end_date = company_fiscal_year.q3.end.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]
    # elif quarter == 4:
    #     adobe_start_date = company_fiscal_year.q4.start.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]
    #     adobe_end_date = company_fiscal_year.q4.end.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]
    # else:
    #     print("You didn't pass in a valid quarter number. Options are 1, 2, 3 or 4")
