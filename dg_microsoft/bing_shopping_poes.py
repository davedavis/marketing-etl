...
from dg_utils.print_debug_headers import print_bing_shopping_poes_headers
...
def get_qtd_bing_shopping_poes(authorization_data, start_date, end_date):
...
def download_report(reporting_download_parameters):
    ...
    print_bing_shopping_poes_headers()
    ...
    for record in report_record_iterable:
        ...
        if record.value('Spend') != '0.00':
            ...
            # Using a try except block for this as some records come back with weird CTR values that can't be
...
def get_report_request(account_id, start_date, end_date):
    ...
    account_performance_report_request = get_bing_shopping_report_request(
...
def get_bing_shopping_report_request(
        account_id,
        aggregation,
    ...
    report_request.ReportName = "Bing-Microsoft Shopping Ads POE Report"
...
def get_bing_shopping_poes(start_date, end_date):
    start_date = start_date
    end_date = end_date
    authenticate(authorization_data)
    get_qtd_bing_shopping_poes(authorization_data, start_date, end_date)
